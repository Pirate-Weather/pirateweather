...
for ncFileName in ncFileRange:
    ...
    for grbType in grbTypes:
        ...
        download_file_path = temp_path + '/gfs.f' + \
            str(ncFileName).zfill(3) + '.' + grbType + '.grb'

        # Download from S3
        s3_filename = 'gfs.' + fDate + '/' + \
            runTime[1:3] + '/atmos/gfs.' + runTime + '.' + \
            grbType + '.0p25.f' + str(ncFileName).zfill(3)
        s3_client.download_file(bucket, s3_filename, download_file_path)

        if grbType == 'pgrb2':
            # Define temp path
            download_file_pathA = temp_path + '/gfs.f' + \
                str(ncFileName).zfill(3) + '.' + grbType + '.grb'
            ...
            pywgrib2_s.wgrib2([download_file_pathA, '-match', matchString,
                              '-append', "-grib_out", download_path_GB_A])
            pywgrib2_s.wgrib2([download_file_pathA, '-rewind_init', download_file_pathA,
                              '-match', 'APCP', '-append', '-grib', download_path_GB_A, '-quit'])
            pywgrib2_s.wgrib2([download_file_pathA, '-rewind_init', download_file_pathA, '-match',
                              'TCDC:entire atmosphere', '-append', '-grib', download_path_GB_A, '-quit'])
            pywgrib2_s.close(download_file_pathA)

        if grbType == 'pgrb2b':
            # Define temp path
            download_file_pathB = temp_path + '/gfs.f' + \
                str(ncFileName).zfill(3) + '.' + grbType + '.grb'
            ...
            pywgrib2_s.wgrib2([download_file_pathB, '-match', '(:DUVB:surface:)',
    ...
    if len(variable.dimensions) == 3:
        if 'PRATE_surface' in name:
            x = chk.createVariable(name, variable.datatype, variable.dimensions, chunksizes=[
                                   160, 2, 4], zlib=True, least_significant_digit=4, complevel=1)
        elif 'APCP_surface' in name:
            x = chk.createVariable(name, variable.datatype, variable.dimensions, chunksizes=[
                                   160, 2, 4], zlib=True, least_significant_digit=4, complevel=1)
        else:
            x = chk.createVariable(name, variable.datatype, variable.dimensions, chunksizes=[
                                   160, 2, 4], zlib=True, least_significant_digit=1, complevel=1)
    else:
