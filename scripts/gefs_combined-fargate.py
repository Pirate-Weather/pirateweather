...
download_path_CK_AVG = download_path + '/' + prod_B + '/' + \
...
for ncFileName in ncFileRange:
    ...
    for grbType in grbTypes:
        if grbType == 'gep':
            ...
            download_path_GB_EN = temp_path + '/gep.f' + \
                str(ncFileName).zfill(3) + '.' + grbType + '.grb' + '.ens'
            download_path_GB_ENP = temp_path + '/gep.f' + \
                str(ncFileName).zfill(3) + '.' + grbType + '.grb' + '.enp'
            download_path_GB_ENP2 = temp_path + '/gep.f' + \
                str(ncFileName).zfill(3) + '.' + grbType + '.grb' + '.enp2'

            for ensFileName in ensFileRange:
                # Define temp path
                download_file_pathA = temp_path + '/gep' + \
                    str(ensFileName).zfill(2) + '.f' + \
                    str(ncFileName).zfill(3) + '.' + grbType + '.grb'

                # Download from S3
                s3_filename = 'gefs.' + fDate + '/' + runTime[1:3] + '/atmos/pgrb2sp25/gep' + str(
                    ensFileName).zfill(2) + '.' + runTime + '.pgrb2s.0p25.f' + str(ncFileName).zfill(3)
                s3_client.download_file(
                    bucket, s3_filename, download_file_pathA)

                # Append precpn to ensemble file and remove member
                pywgrib2_s.wgrib2(
                    [download_file_pathA, '-match', 'APCP', '-append', "-grib_out", download_path_GB_EN])
            ...
            pywgrib2_s.wgrib2(
                [download_path_GB_EN, '-ens_processing', download_path_GB_ENP, '0'])
            pywgrib2_s.close(download_path_GB_ENP)
            matchString = ":(ens spread):"
            pywgrib2_s.wgrib2([download_path_GB_ENP, '-match', matchString,
                              '-set_ext_name', '1', '-append', '-netcdf', download_path_NC_A])

            # Process ensemble using rpn and extract mean
            pywgrib2_s.wgrib2([download_path_GB_EN, '-rewind_init', download_path_GB_EN,
                              '-rpn', '1:>=', '-ens_processing', download_path_GB_ENP2, '0'])
            pywgrib2_s.close(download_path_GB_ENP2)
            matchString = ":(ens mean):"
            pywgrib2_s.wgrib2([download_path_GB_ENP2, '-match', matchString, '-set_prob', '1',
                              '1', '1', '1', '1', '-set_ext_name', '1', '-append', '-netcdf', download_path_NC_A])
        ...
        if grbType == 'geavg':
            # Averaged
            # Define temp paths
            download_file_pathB = temp_path + '/gefs.f' + \
                str(ncFileName).zfill(3) + '.' + grbType + '.grb'
            download_path_GB_B = download_file_pathB + '.earth'
            # Download from S3
            s3_filename = 'gefs.' + fDate + '/' + \
                runTime[1:3] + '/atmos/pgrb2sp25/geavg.' + runTime + \
                '.pgrb2s.0p25.f' + str(ncFileName).zfill(3)
            # print(bucket)
            # print(s3_filename)
            ...
            pywgrib2_s.wgrib2([download_file_pathB, '-match', matchString,
                              '-append', "-grib_out", download_path_GB_B])
            a = pywgrib2_s.wgrib2([download_file_pathB, '-rewind_init', download_file_pathB,
                                  '-match', 'APCP', '-append', '-grib', download_path_GB_B, '-quit'])
        ...
        if len(variable.dimensions) == 3:
            if 'APCP_surface' in name:
                x = chk.createVariable(name, variable.datatype, variable.dimensions, chunksizes=[
                                       80, 7, 7], zlib=True, least_significant_digit=4, complevel=1)
            elif len(variable.dimensions) == 3:
                x = chk.createVariable(name, variable.datatype, variable.dimensions, chunksizes=[
                                       80, 7, 7], zlib=True, least_significant_digit=2, complevel=1)
            else:
